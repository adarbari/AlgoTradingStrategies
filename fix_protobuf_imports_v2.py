...
Version 2: Only removes runtime_version imports, preserves DESCRIPTOR
"""
...
def fix_protobuf_file(file_path):
    """Fix a single protobuf file by removing only runtime_version imports."""
    with open(file_path, 'r') as f:
        content = f.read()
    
    # Remove the runtime_version import line
    content = re.sub(
    ...
    # Remove the runtime_version validation block (but keep everything else)
    content = re.sub(
