...
#   ========================
#   This class provides a singleton interface for retrieving historical tick bars
#   from Rithmic systems using the R | Protocol API.
#
...
from . import request_heartbeat_pb2
from . import response_heartbeat_pb2

from . import request_rithmic_system_info_pb2
from . import response_rithmic_system_info_pb2

from . import request_login_pb2
from . import response_login_pb2

from . import request_logout_pb2
from . import response_logout_pb2

from . import request_tick_bar_replay_pb2
from . import response_tick_bar_replay_pb2

#   ===========================================================================

USAGE   = "HistoricalDataProvider.py connect_point [system_name user_id password exchange symbol]"
USAGE_2 = "  (try wss://rituz00100.rithmic.com:443 for the connect_point)"

#   ===========================================================================

class HistoricalTickBarProvider:
    """
    Singleton class for retrieving historical tick bars from Rithmic systems.
    """
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(HistoricalTickBarProvider, cls).__new__(cls)
        return cls._instance
    
    def __init__(self):
        if not self._initialized:
            self._g_rp_is_done = False
            self._initialized = True
    
    #   ===========================================================================
    #   This routine interprets the msg_buf as a ResponseTickBarReplay

    async def _response_tick_bar_replay_cb(self, msg_buf):
        ...
        if len(msg.rq_handler_rp_code) == 0 and \
            ...
            self._g_rp_is_done = True
            
    #   ===========================================================================
    #   This routine connects to the specified URI and returns the websocket
    #   connection object.

    async def _connect_to_rithmic(self, uri, ssl_context):
        ...
        try:
            ...
            print(f"Invalid handshake with {uri}: {e}")
            sys.exit()
        except websockets.exceptions.ConnectionClosedError as e:
            print(f"Connection to {uri} was closed unexpectedly: {e}")
            sys.exit()
        except websockets.exceptions.InvalidMessage as e:
            print(f"Received invalid message from {uri}: {e}")
            sys.exit()
        except Exception as e:
            print(f"Unexpected error connecting to {uri}: {e}")
            sys.exit()
        return ws

    #   ===========================================================================
    #   This routine sends a heartbeat message to the server.
    ...
    #   ===========================================================================
    #   This routine requests a list of available Rithmic systems.

    async def _list_systems(self, ws):
        ...
        print(f"")
        print(f"   ResponseRithmicSystemInfo :")
        print(f"      =======================")
        ...
        print(f"         infra_type : {rp.infra_type}")
        print(f"        description : {rp.description}")
        print(f"")

    #   ===========================================================================
    #   This routine consumes messages from the websocket connection.

    async def _consume(self, ws):
        max_num_msgs = 1000
        num_msgs = 0

        while num_msgs < max_num_msgs and not self._g_rp_is_done:
            msg_buf = bytearray()
            ...
            if base.template_id == 13:
                ...
                print(f" consumed msg : {msg_type} ({base.template_id})")
                await self._response_tick_bar_replay_cb(msg_buf)
    ...
    async def _rithmic_login(self, ws, system_name, infra_type, user_id, password):
    ...
    async def _replay_tick_bars(self, ws, exchange, symbol):
        ...
        rq.start_index  = 1495260800    # 2020-07-20 12:00:00 EDT
    ...
    async def _rithmic_logout(self, ws):
    ...
    async def _disconnect_from_rithmic(self, ws):
        await ws.close(1000, "see you tomorrow")

    #   ===========================================================================
    #   Public method - This is the main entry point for the class

    def get_historical_data(self, uri, system_name=None, user_id=None, password=None, exchange=None, symbol=None):
        """
        Public method to retrieve historical data from Rithmic.
        
        Args:
            uri (str): The connection URI (e.g., "wss://rituz00100.rithmic.com:443")
            system_name (str, optional): The system name to connect to
            user_id (str, optional): User ID for authentication
            password (str, optional): Password for authentication
            exchange (str, optional): Exchange name
            symbol (str, optional): Symbol to retrieve data for
            
        Returns:
            None: This method prints results to console
        """
        ...
        ws = loop.run_until_complete(self._connect_to_rithmic(uri, ssl_context))

        if system_name is None:
            loop.run_until_complete(self._list_systems(ws))
        else:
            loop.run_until_complete(self._rithmic_login(ws,
                                                      system_name,
                                                      request_login_pb2.RequestLogin.SysInfraType.HISTORY_PLANT,
                                                      user_id,
                                                      password))

            loop.run_until_complete(self._replay_tick_bars(ws, exchange, symbol))

            loop.run_until_complete(self._consume(ws))

            if ws.open:
                print(f"logging out ...")
                loop.run_until_complete(self._rithmic_logout(ws))
                print(f"disconnecting ...")
                loop.run_until_complete(self._disconnect_from_rithmic(ws))
                print(f"done!")
            else:
...
if __name__ == "__main__":
    ...
    if num_args == 2 or num_args == 7:
        ...
        if num_args == 2:
            # Just list systems
            provider = HistoricalTickBarProvider()
            provider.get_historical_data(uri)
        elif num_args == 7:
            ...
            provider = HistoricalTickBarProvider()
            provider.get_historical_data(uri, system_name, user_id, password, exchange, symbol)
    else:
        print(f"{USAGE}")
        print(f"{USAGE_2}")
